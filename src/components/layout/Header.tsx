...
const GameIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"> <path strokeLinecap="round" strokeLinejoin="round" d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.82m5.84-2.56a16.5 16.5 0 0 0-1.636-7.028M15.59 14.37a16.5 16.5 0 0 1 1.636 7.028m-5.84-7.38a16.5 16.5 0 0 0-7.028 1.636M12 1.834v4.82A6 6 0 0 1 15.59 14.37m-3.59 7.38v-4.82a6 6 0 0 1-5.84-7.38m5.84 2.56a16.5 16.5 0 0 1-1.636-7.028M12 1.834a16.5 16.5 0 0 0-7.028 1.636" /> </svg> );
const ToolsIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"> <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.83-5.83M11.42 15.17 7.37 11.12a2.652 2.652 0 0 1 0-3.75L11.12 3.62c.98-.98 2.69-.98 3.67 0L17.25 6m-5.83 9.17-5.83 5.83a2.652 2.652 0 0 1-3.75 0L.62 17.25c-.98-.98-.98-2.69 0-3.67l5.83-5.83M11.42 15.17A2.652 2.652 0 0 0 11.42 15.17Z" /> </svg> );
...
const Header: React.FC<HeaderProps> = ({
  ...
  // No more showHeaderSearch, we show it always!

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
  };

  // This function now writes to the GLOBAL store
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    dispatch(setQuery(value)); // <-- DISPATCH TO REDUX
  };
      ...
      <div className="container-fluid mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-12">
          <form onSubmit={handleSearch} className="header-search w-full">
            <input
              type="text"
              value={globalQuery} // <-- Read from Redux
              onChange={handleInputChange} // <-- Write to Redux
              placeholder="Search games, tools..." // <-- Updated placeholder
